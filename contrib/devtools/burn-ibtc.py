...
"""
burn-ibtc: create a bitcoin burn address
By James C. Stroud
...
Call the program with a template burn address as the only argument::
    % burn-ibtc LZBurnAddressForIbtcPreminexxxxxxx
    LZBurnAddressForIbtcPreminextStefG
For instructions, call the program with no arguments::
    % burn-ibtc
    usage: burn-ibtc TEMPLATE
...
class BurnibtcError(Exception):
    pass


class AlphabetError(BurnibtcError):
    pass
...
def usage():
    print "usage: burn-ibtc TEMPLATE"
    print
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        usage()
    if sys.argv[1] == "test":
        template = "RXBurnAddressForIbtcPreminexxxxxxx"
    else:
        template = sys.argv[1]
    for c in template:
        if c not in ABET:
