...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i ibitcoin-qt.dmg$ | xargs -i cp {} $OUTDIR/bin
    find $TRAVIS_BUILD_DIR -type f | grep -i ibitcoin-core.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/ibitcoin-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/ibitcoin-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/ibitcoind $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/ibitcoin.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/ibitcoin-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/ibitcoin-cli.exe $OUTDIR/bin/
