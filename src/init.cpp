...
#include "config/ibitcoin-config.h"
#endif
...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZibtcThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "ibitcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ibitcoind.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the iBTCCOIN and ziBTC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 8950, 10500));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in iBTCCOIN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in iBTCCOIN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, ibitcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in iBTCCOIN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all iBTCCOIN specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-ibtcstake=<n>", strprintf(_("Enable or disable staking functionality for iBTCCOIN inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zibtcstake=<n>", strprintf(_("Enable or disable staking functionality for ziBTC inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:8950"));
    ...
    strUsage += HelpMessageOpt("-backupzibtc=<n>", strprintf(_("Enable automatic wallet backups triggered after each ziBTC minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zibtcbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic ziBTC backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizeibitcoinamount=<n>     " + strprintf(_("Keep N iBTCCOIN anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 8951, 11000));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The iBTC Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2020-%i The ImperioBitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that iBTCCOIN is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ImperioBitcoin Core is shutting down."));
...
#endif
    // Make sure only a single iBTCCOIN process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ImperioBitcoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("iBTCCOIN version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (proxyArg != "" && proxyArg != "0") {
        CService proxyAddr;
        if (!Lookup(proxyArg.c_str(), proxyAddr, 8950, fNameLookup)) {
    ...
    if (onionArg != "") {
        if (onionArg == "0") { // Handle -noonion/-onion=0
            ...
            CService onionProxy;
            if (!Lookup(onionArg.c_str(), onionProxy, 8950, fNameLookup)) {
    ...
    //iBTCCOIN: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //iBTCCOIN specific: zerocoin and spork DB's
                ...
                // iBTCCOIN: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZiBTCMinted();
                        RecalculateZiBTCSpent();
                    //}
                    RecalculateiBTCCOINSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    //if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        ...
                        // iBTCCOIN: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ImperioBitcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart ImperioBitcoin Core to complete") << "\n";
        ...
        zwalletMain = new CziBTCWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize ziBTCWallet
        uiInterface.InitMessage(_("Syncing ziBTC wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZIbtcBackups = GetBoolArg("-backupzibtc", true);
        pwalletMain->setZIbtcAutoBackups(fEnableZIbtcBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zibtcTracker->Init();
...
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeImperioBitcoinAmount = 2;
...
//    }
//
//    nAnonymizeImperioBitcoinAmount = GetArg("-anonymizeibitcoinamount", 0);
//    if (nAnonymizeImperioBitcoinAmount > 999999) nAnonymizeImperioBitcoinAmount = 999999;
//    if (nAnonymizeImperioBitcoinAmount < 2) nAnonymizeImperioBitcoinAmount = 2;
    ...
    LogPrintf("Anonymize iBTCCOIN Amount %d\n", nAnonymizeImperioBitcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1iBTCCOIN+1000 == (.1iBTCCOIN+100)*10
       10iBTCCOIN+10000 == (1iBTCCOIN+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZibtcThread(threadGroup);
    }
