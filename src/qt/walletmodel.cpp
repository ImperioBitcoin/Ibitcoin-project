...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ibitcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 iBTC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ibitcoin:URI (ibitcoin:XyZ...?message=example)
            {
...
static void NotifyziBTCReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyziBTCReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyziBTCReset.connect(boost::bind(NotifyziBTCReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyziBTCReset.disconnect(boost::bind(NotifyziBTCReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zibtcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
