...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a iBTCCOIN address (e.g. %1)").arg("KHD3HxhEkWGsupqZeN1p3QkvZq7EjQTCQH"));
...
{
    // return if URI is not valid or is no iBTCCOIN: URI
...
{
    // Convert ibitcoin:// to ibitcoin:
    //
    //    Cannot handle this later, because ibitcoin:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open ibitcoin.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "iBTCCOIN.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for iBTCCOIN.lnk
...
{
    return GetAutostartDir() / "ibitcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ibitcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=iBTCCOIN\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ibitcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ibitcoin app to startup item list
