...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& siBTCCOINPercentage, QString& sziBTCPercentage)
{
    int nPrecision = 2;
    ...
    siBTCCOINPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in ibitcoin.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in ibitcoin.conf");
    }
    ...
    // iBTCCOIN Available
    bool showiBTCCOINAvailable = settingShowAllBalances || ibtcAvailableBalance != nTotalBalance;
    bool showWatchOnlyiBTCCOINAvailable = showiBTCCOINAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showiBTCCOINAvailable || showWatchOnlyiBTCCOINAvailable);
    ui->labelBalance->setVisible(showiBTCCOINAvailable || showWatchOnlyiBTCCOINAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlyiBTCCOINAvailable && showWatchOnly);

    // iBTCCOIN Pending
    bool showiBTCCOINPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyiBTCCOINPending = showiBTCCOINPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showiBTCCOINPending || showWatchOnlyiBTCCOINPending);
    ui->labelUnconfirmed->setVisible(showiBTCCOINPending || showWatchOnlyiBTCCOINPending);
    ui->labelWatchPending->setVisible(showWatchOnlyiBTCCOINPending && showWatchOnly);

    // iBTCCOIN Immature
    bool showiBTCCOINImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showiBTCCOINImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showiBTCCOINImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showiBTCCOINImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // iBTCCOIN Locked
    bool showiBTCCOINLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlyiBTCCOINLocked = showiBTCCOINLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showiBTCCOINLocked || showWatchOnlyiBTCCOINLocked);
    ui->labelLockedBalance->setVisible(showiBTCCOINLocked || showWatchOnlyiBTCCOINLocked);
    ui->labelWatchLocked->setVisible(showWatchOnlyiBTCCOINLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("iBTCCOIN")
    updateDisplayUnit();
