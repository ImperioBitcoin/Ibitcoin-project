...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::iBTC);
    ...
    if (!settings.contains("nAnonymizeImperioBitcoinAmount"))
        settings.setValue("nAnonymizeImperioBitcoinAmount", 1000);

    nAnonymizeImperioBitcoinAmount = settings.value("nAnonymizeImperioBitcoinAmount").toLongLong();
...
// Wallet
    ...
    if (!settings.contains("addrProxy"))
        settings.setValue("addrProxy", "127.0.0.1:8950");
    ...
    if (settings.contains("nAnonymizeImperioBitcoinAmount"))
        SoftSetArg("-anonymizeibitcoinamount", settings.value("nAnonymizeImperioBitcoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ibitcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeImperioBitcoinAmount:
            return QVariant(nAnonymizeImperioBitcoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeImperioBitcoinAmount:
            nAnonymizeImperioBitcoinAmount = value.toInt();
            settings.setValue("nAnonymizeImperioBitcoinAmount", nAnonymizeImperioBitcoinAmount);
            emit anonymizeImperioBitcoinAmountChanged(nAnonymizeImperioBitcoinAmount);
            break;
        case CoinControlFeatures:
