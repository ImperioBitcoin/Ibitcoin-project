...
#include "ui_zibtccontroldialog.h"
...
std::set<std::string> ZIbtcControlDialog::setSelectedMints;
std::set<CMintMeta> ZIbtcControlDialog::setMints;

bool CZIbtcControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZIbtcControlDialog::COLUMN_DENOMINATION || column == ZIbtcControlDialog::COLUMN_VERSION || column == ZIbtcControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZIbtcControlDialog::ZIbtcControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZIbtcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZIbtcControlDialog::~ZIbtcControlDialog()
{
    delete ui;
}

void ZIbtcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZIbtcControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZIbtcControlWidgetItem* itemDenom(new CZIbtcControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZIbtcControlWidgetItem *itemMint = new CZIbtcControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZIbtcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZIbtcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZIbtc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZIbtcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZIbtcControlDialog::GetSelectedMints()
{
...
void ZIbtcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
