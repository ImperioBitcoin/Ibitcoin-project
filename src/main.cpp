...
#if defined(NDEBUG)
#error "iBTCCOIN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uibtc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : ziBTC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the ziBTC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 ziBTC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend ziBTC without the correct spend type. txid=%s", __func__,
                tx.GetHash().GetHex());
        }
    }    
...

    {
        ...
        if (tx.IsZerocoinSpend()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : ziBTC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                             tx.GetHash().GetHex()),
                        REJECT_INVALID, "bad-txns-invalid-zibtc");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that ziBTC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 25995.82154782 * COIN;
    } else if (nHeight < 5000 && nHeight >= 0) {
        nSubsidy = 1.00589214 * COIN;
    } else if (nHeight < 150000 && nHeight >= 5001) {
        nSubsidy = 0.10895222 * COIN;
    } else if (nHeight < 250000 && nHeight >= 150001) {
        nSubsidy = 0.08399987 * COIN;
    } else if (nHeight < 500000 && nHeight >= 250001) {
        nSubsidy = 0.05210999 * COIN;
    } else if (nHeight < 550000 && nHeight >= 500001) {
        nSubsidy = 0.04799958 * COIN;
    } else if (nHeight < 650000 && nHeight >= 550001) {
        nSubsidy = 0.14999895 * COIN;
    } else if (nHeight < 700000 && nHeight >= 650001) {
        nSubsidy = 0.11999751 * COIN;
        
    } else {
        nSubsidy = 0.52658745 * COIN;
    }
    return nSubsidy;
}

int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZIbtcStake)
{
    if(!isZIbtcStake){
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from iBTCCOIN
...
{
    RenameThread("ibitcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZiBTCMinted()
{
...
void RecalculateZiBTCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from ziBTC supply
...
bool RecalculateiBTCCOINSupply(int nHeightStart)
{
...
bool UpdateZiBTCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    ...
    CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track ziBTC money supply in the block index
    if (!UpdateZiBTCSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new ziBTC supply for block=%s height=%d", __func__, block.GetHash().GetHex(), pindex->nHeight), REJECT_INVALID);
    ...
    if (fDebug) {
        LogPrintf("ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert iBTCCOIN to ziBTC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (block.GetBlockTime() > GetAdjustedTime() + (block.IsProofOfStake() ? 120 : 7200)) // 2 minute future drift for PoS
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent ziBTC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of ziBTC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    //     return error("%s: ziBTC stake block is less than allowed start height", __func__);

    if (CZIbtcStake* ziBTC = dynamic_cast<CZIbtcStake*>(stake)) {
        CBlockIndex* pindexFrom = ziBTC->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with ziBTC stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: ziBTC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(ziBTC->GetValue()));
        if (nChecksum200 != ziBTC->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, ziBTC->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZiBTC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked ziBTC fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ibtcInputs;
        std::vector<CTxIn> ziBTCInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                ziBTCInputs.push_back(stakeIn);
            }else{
                ibtcInputs.push_back(stakeIn);
            }
        }
        const bool hasiBTCInputs = !ibtcInputs.empty();
        const bool hasZiBTCInputs = !ziBTCInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasiBTCInputs)
                    ...
                    for (const CTxIn& ibtcIn : ibtcInputs){
                        if(ibtcIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : ibtcInputs) {
            ...
            // Now that this loop if completed. Check if we have ziBTC inputs.
            if(hasZiBTCInputs){
                for (const CTxIn& zIbtcInput : ziBTCInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zIbtcInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zibtc");
                    ...
                    //Accumulator accumulator(spend.getDenomination(), bnAccumulatorValue);
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d ziBTC mints and %d ziBTC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // iBTCCOIN: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zibtc", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zibtc", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
