...
iBTCCOIN now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ibitcoin_address\> \<monthly_payment_ibitcoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ iBTCCOIN, _12_ times over the course of a year totaling _24000_ iBTCCOIN.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<ibitcoin_address\> \<monthly_payment_ibitcoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` iBTCCOIN.
